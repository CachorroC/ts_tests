import { fixDemandado } from '../middleware/polishProcesos.js';
import rows from '../out/rows.getProcesos.json';
import { intConsultaNumeroRadicacion } from '../src/procesos.js';
import * as fsnp from 'fs';
import * as fsp from 'fs/promises';

const Consultas: intConsultaNumeroRadicacion[] = [
  {
    ...
    procesos: [
      {
        ...
        departamento: 'BOGOTÁ',
        sujetosProcesales:
          'Demandante: BANCOLOMBIA S.A. | Demandado: MARCO TULIO SANCHEZ MEDINA | Demandado: SANDRA LEONOR PAEZ MURCIA ',
        esPrivado: false,
        cantFilas: -1,
      },
    ],
    ...
    procesos: [
      {
        ...
        fechaUltimaActuacion: '2023-04-25T00:00:00',
        despacho:
          'JUZGADO 011 CIVIL MUNICIPAL DE EJECUCIÓN DE SENTENCIAS DE BOGOTÁ ',
        departamento: 'BOGOTÁ',
        sujetosProcesales:
...
const Demandados = Consultas.map(
  ( consulta, index ) => {
    if ( consulta.procesos.length === 0 ) {
      return { llaveProceso: consulta.parametros.numero };
    }
    const Procesos = consulta.procesos;
    const llaveP = consulta.parametros.numero;

    const nombre = fixDemandado( Procesos[ 0 ].sujetosProcesales );
    const nombre2 = Procesos.map( ( proceso ) => {
      return fixDemandado( proceso.sujetosProcesales );
    } ).toString();
    const llaveProceso = Procesos[ 0 ].llaveProceso
      ? Procesos[ 0 ].llaveProceso
      : undefined;
...
console.log( JSON.stringify( Demandados ) );
fsp.writeFile(
  'out/Demandados.json', JSON.stringify( Demandados )
).then( ( success ) => {
...
const DemandadosWP = Consultas.map(
  ( consulta, index ) => {
    if ( consulta.procesos.length === 0 ) {
      return {};
    }
...
console.log( JSON.stringify( DemandadosWP ) );
fsp.writeFile(
  'out/DemandadoswithoutProcesos.json', JSON.stringify( DemandadosWP )
).then( ( success ) => {
  return console.log( success );
}, ( error ) => {
  return console.log( error );
