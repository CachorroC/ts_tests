        ...
        procesos: [
            {
                ...
                departamento: 'BOGOTÁ',
                sujetosProcesales: 'Demandante: BANCOLOMBIA S.A. | Demandado: MARCO TULIO SANCHEZ MEDINA | Demandado: SANDRA LEONOR PAEZ MURCIA ',
                esPrivado: false,
                cantFilas: -1,
            },
        ],
...
const Demandados = Consultas.map((consulta, index) => {
    ...
    const llaveProceso = Procesos[0].llaveProceso
        ...
        : undefined;
    const idProcesos = Procesos.map((proceso) => {
...
fsp.writeFile('out/Demandados.json', JSON.stringify(Demandados)).then((success) => {
    return console.log(success);
}, (error) => {
    return console.log(error);
});
const DemandadosWP = Consultas.map((consulta, index) => {
    if (consulta.procesos.length === 0) {
        return {};
    }
    const Procesos = consulta.procesos;
...
fsp.writeFile('out/DemandadoswithoutProcesos.json', JSON.stringify(DemandadosWP)).then((success) => {
    return console.log(success);
}, (error) => {
    return console.log(error);
});
