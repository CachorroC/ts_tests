...
export const rows: intConsultaNumeroRadicacion[] = [];
export async function fetchProceso(llaveProceso: string) {
  ...
  const rawData = (await res.json()) as intConsultaNumeroRadicacion;
  fs.writeFile(
    "lib/rawData.ts",
    `const rawData = ${JSON.stringify(rawData)}; export default rawData;`
  );
  fs.writeFile(
    "lib/rows.ts",
    `const dataRows = ${JSON.stringify(rows)}; export default dataRows;`
  );
  return rawData;
}
...
export const getProcesos = llaves.map((llave, index) => {
  ...
  setTimeout(() => {
    return fetchProceso(llave).then(
      (procesos) => {
...
export const missingProcesos: number[] = [];
...
const polishProcesos = getProcesosfromRadicados.forEach((procesos, index) => {
  ...
  procesos.forEach((proceso) => {
    ...
    fs.writeFile(
      "lib/cleanProcesos.ts",
      `const cleanProcesos = ${JSON.stringify(
        nonMissingProcesos
...
export const Procesos: intProceso[] = nonMissingProcesos.map(
  (proceso: intProceso) => {
    const Proceso = {
